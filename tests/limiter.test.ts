...
const MINUTE = 60_000;
const DWELL_MS = CONTROL.minDwellMinutes * MINUTE;
const LONG_AGO = NOW - 24 * 60 * MINUTE;
...
describe('increases', () => {
  ...
  it('are not gated by the dwell timer', () => {
    ...
    const oneMinuteAgo = NOW - 1 * MINUTE;
  ...
  it('restart the dwell clock', () => {
    ...
    assert.equal(outcome.lastChangeAt, NOW);
  });
});
...
describe('decreases', () => {
  ...
  it('take six steps and an hour to walk the whole range down', () => {
    ...
    let lastChangeAt: number | undefined = LONG_AGO;

    for (let minute = 0; minute <= 60; minute += 1) {
      const outcome = limit(wants(20), level, lastChangeAt, NOW + minute * MINUTE);
  ...
  it('give way at once to a demand that reverses direction', () => {
    ...
    const reversed = limit(wants(80), stepped.level, stepped.lastChangeAt, NOW + MINUTE);
...
describe('dwell', () => {
  it('suppresses a step down until it has elapsed', () => {
    const outcome = limit(wants(20), 80, NOW - 9 * MINUTE, NOW);
  ...
  it('permits a step down exactly on the boundary', () => {
    const outcome = limit(wants(20), 80, NOW - DWELL_MS, NOW);
  ...
  it('is measured from the last change, not the last evaluation', () => {
    ...
    for (let tick = 0; tick < 19; tick += 1) {
      const evaluation = limit(wants(20), 80, lastChangeAt, NOW + tick * 30_000);
      assert.equal(evaluation.write, false);
      assert.equal(evaluation.lastChangeAt, lastChangeAt, 'an evaluation must not restart the dwell');
    }

    const onceElapsed = limit(wants(20), 80, lastChangeAt, lastChangeAt + DWELL_MS);
  ...
  it('reports a suppressed change rather than dropping it silently', () => {
    const outcome = limit(wants(20), 80, NOW - MINUTE, NOW);
...
describe('no change needed', () => {
  it('issues no write when the demand is already the current level', () => {
    ...
    assert.equal(outcome.write, false);
    assert.equal(outcome.lastChangeAt, LONG_AGO);
  });
});
...
describe('the sleep cap', () => {
  ...
  it('bypasses the dwell timer entirely', () => {
    ...
    const outcome = limit(wants(70, true), 70, NOW - MINUTE, NOW);
  ...
  it('does not restart the dwell clock', () => {
    ...
    const lastChangeAt = NOW - 5 * MINUTE;

    const outcome = limit(wants(30, true), 70, lastChangeAt, NOW);

    assert.equal(outcome.lastChangeAt, lastChangeAt);
  });
  ...
  it('clamps the output and never the demand behind it', () => {
    ...
    const released = limit(wants(80, false), capped.level, capped.lastChangeAt, NOW + MINUTE);
